...
const main = async () => {
  ...
  for (const target of targets) {
    ...
    if (typeof tarballUrl !== "string") {
      throw new Error(
        `No dist.tarball found for ${target.npmPackage}@${sqliteVecVersion}`,
      );
    }
    ...
    if (!releaseArtifact) {
      throw new Error(
        `Upstream release manifest is missing ${expectedArtifactName}`,
      );
    }
    ...
    try {
      execFileSync("tar", ["-xzf", tarballPath, "-C", workingDir], {
        stdio: "pipe",
      });
      ...
      if (!existsSync(extractedBinary)) {
        throw new Error(
          `Expected ${target.sourceBinary} inside ${target.npmPackage} tarball`,
        );
      }
      ...
      console.log(
        `Vendored ${target.npmPackage}@${sqliteVecVersion} -> ${destinationPath}`,
      );
    } finally {
